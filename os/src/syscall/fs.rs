...
pub fn sys_linkat(old_name: *const u8, new_name: *const u8) -> isize {
    ...
    let old_inode = match crate::fs::ROOT_INODE.find(old_path.as_str()) {
        Some(i) => i,
        None => return -1,
    };
    if crate::fs::ROOT_INODE.link(new_path.as_str(), &old_inode) {
        0
    } else {
        -1
    }
...
pub fn sys_unlinkat(name: *const u8) -> isize {
    ...
    if crate::fs::ROOT_INODE.unlink(path.as_str()) {
        0
    } else {
        -1
    }
