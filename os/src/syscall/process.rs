...
/// get time with second and microsecond, writing result into user TimeVal
/// Uses page table translation to handle the case where TimeVal crosses pages.
...
/// Anonymous mmap: map [start, start+len) into current task with prot flags.
...
/// Anonymous munmap: unmap [start, start+len) from current task.
...
/// Spawn a new process to execute the program at `path`.
/// On success, returns child's pid; on invalid filename, returns -1.
pub fn sys_spawn(path: *const u8) -> isize {
    ...
    let data = match get_app_data_by_name(path_str.as_str()) {
        Some(d) => d,
        None => return -1,
    };
...
// Set task priority.
// Returns new priority on success, or -1 on invalid prio.
